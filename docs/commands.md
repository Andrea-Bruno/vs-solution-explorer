...
- **Open Solution as Multi-Root Workspace**: writes a `.code-workspace` next to the solution whose folders are every project's directory (plus the solution's own folder when projects live outside it, e.g. sibling repositories) and offers to reload VS Code into it. A solution often spans several folders/repositories, and a multi-root workspace makes all of them part of a single window. When every project lives inside the solution folder, the workspace file keeps that one folder and nothing is duplicated. This is the file-based variant of the automatic behavior: with `csharpSolutionExplorer.workspace.autoIncludeProjectFolders` on (the default) the same project directories are added as workspace folders automatically when the window opens, with no reload — use this command when you want the layout saved as a shareable `.code-workspace` file instead.
