...
| `csharpSolutionExplorer.workspace.autoIncludeProjectFolders` | `true` | When the solution references projects outside the opened folder (shared libraries in sibling folders or other repositories), automatically add each such project directory as a workspace folder, so search, source control, coding agents and the language server treat the whole solution as part of the workspace. Runs when the window opens and when this setting changes; it only adds folders and never removes any. Turn off to keep only the folder you opened. |
