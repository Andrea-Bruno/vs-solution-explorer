...
[![VS Marketplace](https://badgen.net/vs-marketplace/v/andreabruno.csharp-solution-explorer-community?label=VS%20Marketplace&color=0066b8)](https://marketplace.visualstudio.com/items?itemName=andreabruno.csharp-solution-explorer-community)
...
*A community fork of [Jomblebee's C# Solution Explorer](https://github.com/Jomblebee/csharp-solution-explorer)
(MIT), focused on the Visual Studio workflow.*

![VS Solution Explorer Tree View](resources/screenshots/tree-view.png)
...
- **Every project is a workspace folder**: when a solution references projects outside the opened folder (shared libraries in sibling folders or other repositories), each project directory is added as a workspace folder automatically, so search, source control, coding agents and the language server treat the whole solution as part of the workspace — no "outside the workspace" refusals. On by default (`csharpSolutionExplorer.workspace.autoIncludeProjectFolders`); the **Open Solution as Multi-Root Workspace** command does the same as a saved `.code-workspace` file — [see below](#the-whole-solution-as-one-workspace).
...
By default the extension removes that limit automatically: when the window opens, every project directory of the solution that is not already inside the opened folder is added as a **workspace folder** (`csharpSolutionExplorer.workspace.autoIncludeProjectFolders`, on by default). No reload, no setup — the whole solution becomes the workspace, and search, IntelliSense, source control and coding agents see all of it. It only ever adds folders and never removes any, and it skips projects already inside the workspace, so it leaves a tidy tree.

If you would rather keep the layout as a file you can commit and share, the **Open Solution as Multi-Root Workspace** command (right-click the solution node, or the Command Palette) writes a `.code-workspace` next to your `.sln`/`.slnx` whose folders are the directories of **every project the solution references**, and asks you to reload the window into it.

Two rules keep the workspace tidy:

- **All projects inside the solution's own folder** → nothing is added, the single folder stays as-is.
- **Projects outside it** (shared libraries, sibling repositories, …) → each one is added as its own workspace root, so you never lose your bearings.

There is a practical payoff for **AI coding agents** too. Agents are confined to the workspace and refuse any operation that would touch a file outside it — so with only the solution's own folder open, an agent literally cannot edit the shared projects that sit elsewhere. Once every project is a workspace folder, agents have full read/write access to the entire solution, with no "outside the workspace" refusals.
